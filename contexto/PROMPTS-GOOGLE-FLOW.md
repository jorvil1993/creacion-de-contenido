# Prompts de Google Flow — banco de material

Material de apoyo (B-roll, PiP) para `40-GUIONES-VIRALES.md`.

**Cada prompt está completo. Copiar, pegar, generar.** No hay que agregarle nada.
...
Para los 8 clips de producto (`P01`–`P08`) usar **`Frames to Video`**: subir la
foto real y pegar el prompt. Todo lo demás es `Text to Video`.

## Presupuesto — 3 cuentas × 1.000 créditos

Los créditos **no se combinan entre cuentas**: son 3 bolsas independientes. Cada
generación (8s) cuesta 20 créditos en Fast → 50 clips por cuenta.

| Cuenta | Qué generar | Clips | Créditos | Reserva |
|---|---|---:|---:|---:|
| **A** | Ambiente 1 y 2 | 18 | 360 | 640 |
| **B** | Ambiente 3 + producto | 22 | 440 | 560 |
| **C** | Segundas versiones | 20 | 400 | 600 |

La reserva es alta a propósito: con los fallos que ya viste, se descarta cerca de
1 de cada 2 clips, sobre todo en producto.
...
### `scroll` — el pulgar infinito · usado en 10 guiones
```
...
### `noche` — la cara iluminada por el celular · 6 guiones
```
...
### `libros` — la pila pesada · 8 guiones
```
...
### `biblioteca` — estanterías infinitas · 7 guiones
```
...
### `abandonado` — el libro de la página 30 · 5 guiones
```
...
# AMBIENTE 2 — alto uso (Cuenta A)

### `regalo` — la caja con moño · 4 guiones
```
...
### `tiempo` — el tiempo que se va · 3 guiones
```
...
### `ninos-pantalla` — el niño a las 11 de la noche · 3 guiones
```
...
### `ninos-leyendo` — el niño que sí lee · 3 guiones
```
...
### `sol` — el sol que ciega la pantalla · 2 guiones
```
...
### `cama` — la cama acogedora · 2 guiones
```
...
# AMBIENTE 3 — específicos (Cuenta B)

### `agua` — las gotas · guion 28
```
...
### `piscina` — el verano · guiones 4 y 28
```
...
# PRODUCTO — `Frames to Video` (Cuenta B)
...
Esto define qué prompt usar. **No mezclar**: un Paperwhite con pantalla a color es
tan falso como un logo inventado.

| Pantalla | Modelos |
|---|---|
| **Blanco y negro** | Kindle Basic · Paperwhite (16GB, 32GB Signature) · Paperwhite Kids · Scribe B/N |
| **A color** | Kindle Colorsoft · Kobo Clara Colour · Kobo Libra Colour · Scribe Colorsoft |
...
| `P06` | `kindle-paperwhite/frontal-raspberry.jpg` | B/N | Regalo |
| `P07` | `paperwhite-kids/frontal.png` | B/N | Niños |
| `P08` | `colorsoft-32gb/frontal.png` | **Color** | Kindle a color |
| `P09` | `kobo-libra-colour-32-gb/frontal.png` | **Color** | Kobo con botones + color |
| `P10` | `kobo-clara-colour-16gb/frontal.png` | **Color** | Kobo compacta a color |
| `P11` | `varios-modelos-en-la-misma-foto/frontal.png` | — | Cajas selladas, stock real |

Todas las rutas cuelgan de `assets/productos/`.

`kindle-paperwhite/` no tiene `frontal.png`: tiene una foto por color
(`frontal-negro.jpg`, `frontal-jade.jpg`, `frontal-raspberry.jpg`). El jade es el
verde matcha que se te viralizó.

⚠️ **`P11` es la excepción a la regla del logo.** Esa foto son cajas de producto
reales, con las impresiones de Amazon y Kobo. Ahí los logos **son legítimos y hay
que conservarlos** — es tu stock real y da confianza. La prohibición de logos
aplica a los aparatos, no a las cajas.

### `P01` — lectura en cama de noche
```
...
### `P03` — a pleno sol
```
Animate this image. The device sits on an outdoor table under harsh bright midday sun, tropical greenery blurred behind, sunlight moving across its surface as leaf shadows drift over it. Slight handheld camera movement, lens flare, high contrast, natural editorial photography. Keep the device exactly as shown: do not alter its shape, proportions, colour or body. Its frame stays completely smooth, blank and unmarked, with no brand name, no logo, no lettering and no engraving anywhere on it. Its screen is a black and white electronic ink display showing a page of a Spanish-language novel: several justified paragraphs of small dark grey Spanish body text on a light warm grey background, with clean margins, exactly like printed prose, with no colour whatsoever on the screen, perfectly readable in direct sunlight with no glare washing it out. That page is a flat image rendered on the glass, never physical paper: the screen stays perfectly rigid and flat at all times, and nothing ever lifts, peels, curls, flies or emerges from it. No loose sheets, no paper, no book pages turning. The content on the screen is completely static and frozen: it never changes, never scrolls, and there is no page-turn animation, no sliding, no fading, no dissolve and no transition of any kind, because a real e-ink screen does not animate smoothly. It is a solid flat slab, not a book, and it never bends, folds or opens. No added text outside the screen, no logos, no watermark. No dialogue, no voiceover, outdoor summer ambience only.
```

### `P04` — luz cálida en la oscuridad
```
Animate this image. The device lies in near-total darkness, its warm amber screen glow the only light source, gently illuminating the surface beneath it. A hand enters frame and shifts it slightly. Camera almost static, very shallow depth of field, cinematic low-key lighting, natural editorial photography. Keep the device exactly as shown: do not alter its shape, proportions, colour or body. Its frame stays completely smooth, blank and unmarked, with no brand name, no logo, no lettering and no engraving anywhere on it. Its screen is a black and white electronic ink display showing a page of a Spanish-language novel: several justified paragraphs of small dark grey Spanish body text on a warm amber-lit background, with clean margins, exactly like printed prose, with no colour whatsoever on the screen. That page is a flat image rendered on the glass, never physical paper: the screen stays perfectly rigid and flat at all times, and nothing ever lifts, peels, curls, flies or emerges from it. No loose sheets, no paper, no book pages turning. The content on the screen is completely static and frozen: it never changes, never scrolls, and there is no page-turn animation, no sliding, no fading, no dissolve and no transition of any kind, because a real e-ink screen does not animate smoothly. It is a solid flat slab, not a book, and it never bends, folds or opens. No added text outside the screen, no logos, no watermark. No dialogue, no voiceover, silent night ambience.
```

### `P05` — liviano, una sola mano
```
Animate this image. The device is held up in a single hand, tilted slowly to show how thin and light it is, warm window light from the side, softly blurred home interior behind. Smooth slow camera movement, shallow depth of field, natural editorial photography. Keep the device exactly as shown: do not alter its shape, proportions, colour or body. Its frame stays completely smooth, blank and unmarked, with no brand name, no logo, no lettering and no engraving anywhere on it. Its screen is a black and white electronic ink display showing a page of a Spanish-language novel: several justified paragraphs of small dark grey Spanish body text on a light warm grey background, with clean margins, exactly like printed prose, with no colour whatsoever on the screen. That page is a flat image rendered on the glass, never physical paper: the screen stays perfectly rigid and flat at all times, and nothing ever lifts, peels, curls, flies or emerges from it. No loose sheets, no paper, no book pages turning. The content on the screen is completely static and frozen: it never changes, never scrolls, and there is no page-turn animation, no sliding, no fading, no dissolve and no transition of any kind, because a real e-ink screen does not animate smoothly. It is a solid flat slab, not a book, and it never bends, folds or opens. No added text outside the screen, no logos, no watermark. No dialogue, no voiceover, subtle room tone only.
```

### `P06` — como regalo · pantalla B/N
```
Animate this image. The device rests on a table beside a satin ribbon and soft festive bokeh lights, warm golden hour light, hands entering frame to slide it gently toward the camera as a gift. Slow push in, shallow depth of field, natural editorial photography, warm celebratory mood. Keep the device exactly as shown: do not alter its shape, proportions, colour or body. Its frame stays completely smooth, blank and unmarked, with no brand name, no logo, no lettering and no engraving anywhere on it. Its screen is a black and white electronic ink display showing a page of a Spanish-language novel: several justified paragraphs of small dark grey Spanish body text on a light warm grey background, with clean margins, exactly like printed prose, with no colour whatsoever on the screen. That page is a flat image rendered on the glass, never physical paper: the screen stays perfectly rigid and flat at all times, and nothing ever lifts, peels, curls, flies or emerges from it. No loose sheets, no paper, no book pages turning. The content on the screen is completely static and frozen: it never changes, never scrolls, and there is no page-turn animation, no sliding, no fading, no dissolve and no transition of any kind, because a real e-ink screen does not animate smoothly. It is a solid flat slab, not a book, and it never bends, folds or opens. No added text outside the screen, no logos, no watermark. No dialogue, no voiceover, soft ribbon sounds only.
```

### `P07` — niños
```
Animate this image. The device is held by small child-sized hands in a bright cozy bedroom, warm afternoon daylight, a soft blanket and stuffed toy blurred in the background. Gentle handheld camera, shallow depth of field, natural editorial photography, warm and hopeful mood. Hands only, no faces. Keep the device exactly as shown: do not alter its shape, proportions, colour or body. Its frame stays completely smooth, blank and unmarked, with no brand name, no logo, no lettering and no engraving anywhere on it. Its screen is a black and white electronic ink display showing a page of a Spanish-language children's story: several short justified paragraphs of dark grey Spanish body text on a light warm grey background, with clean margins, exactly like printed prose, with no colour whatsoever on the screen. That page is a flat image rendered on the glass, never physical paper: the screen stays perfectly rigid and flat at all times, and nothing ever lifts, peels, curls, flies or emerges from it. No loose sheets, no paper, no book pages turning. The content on the screen is completely static and frozen: it never changes, never scrolls, and there is no page-turn animation, no sliding, no fading, no dissolve and no transition of any kind, because a real e-ink screen does not animate smoothly. It is a solid flat slab, not a book, and it never bends, folds or opens. No added text outside the screen, no logos, no watermark. No dialogue, no voiceover, quiet room tone only.
```

### `P08` — Kindle a color · pantalla COLOR
```
Animate this image. The device rests on a clean light surface as warm directional light slowly sweeps across it, making its colour saturate and glow. Slow smooth camera orbit, shallow depth of field, natural editorial photography, crisp and vivid. Keep the device exactly as shown: do not alter its shape, proportions, colour or body. Its frame stays completely smooth, blank and unmarked, with no brand name, no logo, no lettering and no engraving anywhere on it. Its screen is a colour electronic ink display, with the soft muted pastel tones typical of colour e-ink rather than the vivid saturation of an LCD, showing a page of a Spanish-language illustrated book: a colour illustration above several justified paragraphs of small dark Spanish body text, with clean margins, exactly like a printed page. That page is a flat image rendered on the glass, never physical paper: the screen stays perfectly rigid and flat at all times, and nothing ever lifts, peels, curls, flies or emerges from it. No loose sheets, no paper, no book pages turning. The content on the screen is completely static and frozen: it never changes, never scrolls, and there is no page-turn animation, no sliding, no fading, no dissolve and no transition of any kind, because a real e-ink screen does not animate smoothly. It is a solid flat slab, not a book, and it never bends, folds or opens. No added text outside the screen, no logos, no watermark. No dialogue, no voiceover, subtle ambient tone only.
```

### `P09` — Kobo Libra Colour, botones físicos · pantalla COLOR
```
Animate this image. The device is held in one hand against a softly blurred warm interior background, the thumb resting on the physical page-turn buttons along its raised side grip, tilting slightly to catch the light on its edge. Slow smooth camera orbit, shallow depth of field, natural editorial photography, clean minimal composition. Keep the device exactly as shown: do not alter its shape, proportions, colour, body or its asymmetric grip with physical buttons. Its frame stays completely smooth, blank and unmarked, with no brand name, no logo, no lettering and no engraving anywhere on it. Its screen is a colour electronic ink display, with the soft muted pastel tones typical of colour e-ink rather than the vivid saturation of an LCD, showing a page of a Spanish-language novel with a few passages highlighted in soft yellow and pink: justified paragraphs of small dark Spanish body text on a light warm background, with clean margins, exactly like printed prose. That page is a flat image rendered on the glass, never physical paper: the screen stays perfectly rigid and flat at all times, and nothing ever lifts, peels, curls, flies or emerges from it. No loose sheets, no paper, no book pages turning. The content on the screen is completely static and frozen: it never changes, never scrolls, and there is no page-turn animation, no sliding, no fading, no dissolve and no transition of any kind, because a real e-ink screen does not animate smoothly. It is a solid flat slab, not a book, and it never bends, folds or opens. No added text outside the screen, no logos, no watermark. No dialogue, no voiceover, subtle room tone and a soft button click only.
```

### `P10` — Kobo Clara Colour, compacta · pantalla COLOR
```
Animate this image. The device rests on a light wooden table beside a cup of coffee, warm morning window light moving gently across it, a hand entering frame to pick it up and hold it comfortably in one palm to show how compact it is. Slow push in, shallow depth of field, natural editorial photography, warm and inviting. Keep the device exactly as shown: do not alter its shape, proportions, colour or body. Its frame stays completely smooth, blank and unmarked, with no brand name, no logo, no lettering and no engraving anywhere on it. Its screen is a colour electronic ink display, with the soft muted pastel tones typical of colour e-ink rather than the vivid saturation of an LCD, showing a page of a Spanish-language novel with a small colour cover thumbnail: justified paragraphs of small dark Spanish body text on a light warm background, with clean margins, exactly like printed prose. That page is a flat image rendered on the glass, never physical paper: the screen stays perfectly rigid and flat at all times, and nothing ever lifts, peels, curls, flies or emerges from it. No loose sheets, no paper, no book pages turning. The content on the screen is completely static and frozen: it never changes, never scrolls, and there is no page-turn animation, no sliding, no fading, no dissolve and no transition of any kind, because a real e-ink screen does not animate smoothly. It is a solid flat slab, not a book, and it never bends, folds or opens. No added text outside the screen, no logos, no watermark. No dialogue, no voiceover, quiet morning room tone only.
```

### `P11` — cajas selladas, stock real · sin pantalla
```
...
# Segundas versiones (Cuenta C)

Los clips que aparecen en 5-10 guiones se van a ver repetidos. Generar una
segunda versión de estos, con el **mismo prompt** — Veo nunca repite resultado:

`scroll` · `ojos` · `noche` · `libros` · `biblioteca` · `abandonado` ·
`notificaciones` · `insomnio` · `regalo` · `caja` · `vitrina` · `tiempo` ·
`bateria` · `entrega` · `ninos-pantalla` · `ninos-leyendo` · `sol` · `cama` ·
`P01` · `P02`

Guardarlas con sufijo: `scroll-2.mp4`, `noche-2.mp4`, `P01-2.mp4`.

---

# Antes de guardar cada clip

| Revisar | Descartar si |
|---|---|
| Marco del aparato | Hay cualquier garabato, letra o símbolo. Debe estar liso y vacío |
| Pantalla | Sale una hoja, se dobla, se abre como libro, o algo emerge del cristal |
| Texto en pantalla | El español se ve como galimatías evidente en primer plano |
| Manos | Dedos de más, fundidos, muñecas imposibles |
| En el celular | Mirarlo en vertical en el teléfono, no en el monitor |

Sobre el texto en español: Veo no escribe bien en ningún idioma. En plano medio o
lejano se lee como prosa española y funciona; en primer plano cerrado se nota que
las palabras están mal. Si el clip es un PiP pequeño, sirve igual — lo que importa
es que **parezca** una página de novela y no una hoja de papel.

Si un concepto falla 3 veces seguidas, cambiar el encuadre (más cerrado suele
salvarlo) o resolverlo con foto fija, que el pipeline ya soporta.
...
```
creacion-de-contenido\assets\generado\video\manual\
```

Con el nombre exacto del prompt, en minúsculas:

```
scroll.mp4          ojos.mp4           noche.mp4
libros.mp4          biblioteca.mp4     abandonado.mp4
notificaciones.mp4  insomnio.mp4       regalo.mp4
caja.mp4            vitrina.mp4        tiempo.mp4
bateria.mp4         entrega.mp4        ninos-pantalla.mp4
ninos-leyendo.mp4   sol.mp4            cama.mp4
...
Esa ruta espeja la convención de `video/auto/` (caché de LTX) y
`generado/manual/` (imágenes puestas a mano). Los nombres `noche`, `sol`, `cama`,
`cafe`, `viaje`, `libros`, `biblioteca`, `agua`, `tina`, `regalo`, `caja` y
`bateria` coinciden con etiquetas que el pipeline usa hoy para imágenes fijas
(`config.PALABRAS_A_TAGS`), así que guardarlos así deja el terreno listo para
cuando conectemos el video a ese mismo sistema.

> ✅ **El pipeline SÍ lee esta carpeta.** Si existe un clip en
> `assets/generado/video/manual/<tag>.mp4` cuyo nombre coincide con una
> etiqueta de `config.PALABRAS_A_TAGS`, el pipeline lo inserta como **B-roll
> a pantalla completa** — tu video desaparece por 2-3 segundos y se ve el
> clip, pero tu voz sigue sonando y los subtítulos siguen visibles. Es la
> prioridad más alta: si hay clip manual, no se genera nada (ni LTX ni Flux)
> ni se busca foto del catálogo para ese concepto.
>
> La función que lo busca es `version_manual_video()` en `f9_generar.py`,
> y el tipo de evento es `"broll"` con `"broll_fullscreen": True`.

- La carpeta no existe todavía: crearla al bajar el primer clip.
- `.gitignore` ignora `*.mp4` → no se suben a git, el respaldo es OneDrive.
  Con las segundas versiones, ~55 clips ≈ 700 MB.
- Anotar en un `_registro.txt` qué clip salió de qué cuenta, para saber dónde
  quedan créditos al regenerar.

---

Precios verificados en [Google Veo Pricing 2026](https://diyai.io/ai-tools/video-generation/google-veo-pricing/) y [Veo 3 Pricing 2026](https://www.veo3ai.io/blog/veo-3-pricing-2026). Cambian sin aviso: confirmar en la pantalla de Flow antes de una tanda grande.
